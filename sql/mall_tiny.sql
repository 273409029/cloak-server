...
drop table if EXISTS  `cloak_domain_info` ;
CREATE TABLE `cloak_domain_info` (
                                     `id` int NOT NULL AUTO_INCREMENT,
                                     `domain_name` varchar(255) DEFAULT NULL COMMENT '为用户分配的域名',
                                     `description` varchar(2048) DEFAULT NULL COMMENT '为用户分配的域名',
                                     `is_check` tinyint(1) NOT NULL DEFAULT '0' COMMENT '是否正在审核中',
                                     `mask_redirect_url` varchar(2048) DEFAULT NULL COMMENT '开启审核 && 黑名单用户 && 匹配规则未命中重定向地址',
                                     `white_list_redirect_url` varchar(2048) DEFAULT NULL COMMENT '白名单命中重定向地址',
                                     `match_rule` text COMMENT '为用户分配的域名',
                                     `tenant_id` int NOT NULL COMMENT '租户id',
                                     `create_time` datetime DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
                                     `update_time` datetime DEFAULT CURRENT_TIMESTAMP COMMENT '更新时间',
                                     `is_delete` tinyint(1) NOT NULL DEFAULT '0' COMMENT '软删除',
                                     PRIMARY KEY (`id`) USING BTREE
) ENGINE=InnoDB AUTO_INCREMENT=4 DEFAULT CHARSET=utf8  COMMENT='域名信息表'


drop table if EXISTS  `cloak_request_logs` ;
CREATE TABLE `cloak_request_logs` (
                                      `id` int NOT NULL AUTO_INCREMENT,
                                      `tenant_id` int DEFAULT NULL COMMENT '租户id',
                                      `domain_name` varchar(255) NOT NULL COMMENT '请求的域名',
                                      `request_ip` varchar(45) NOT NULL COMMENT '请求IP',
                                      `ip_region` varchar(10240) DEFAULT NULL COMMENT 'IP所属地区',
                                      `header_info` text COMMENT 'IPheader_info地区',
                                      `user_agent` varchar(1024) DEFAULT NULL COMMENT '请求的用户代理（User-Agent）',
                                      `check_status` tinyint(1) NOT NULL DEFAULT '0' COMMENT '域名是否开启审核开关，0为关闭，1为开启',
                                      `rule_matched` varchar(2048) DEFAULT NULL COMMENT '匹配规则命中，未命中为NULL',
                                      `redirect_url` varchar(255) DEFAULT NULL COMMENT '重定向地址，未重定向为NULL',
                                      `create_time` datetime DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
                                      `update_time` datetime DEFAULT CURRENT_TIMESTAMP COMMENT '更新时间',
                                      `is_delete` tinyint(1) NOT NULL DEFAULT '0' COMMENT '软删除',
                                      PRIMARY KEY (`id`) USING BTREE
) ENGINE=InnoDB AUTO_INCREMENT=6 DEFAULT CHARSET=utf8  COMMENT='请求日志表'

drop table if EXISTS `cloak_ip_list`;
...
CREATE TABLE
    `cloak_ip_list` (
                        ...
                        `request_ip` varchar(45) NOT NULL COMMENT '请求IP',
                        `is_white` tinyint(1) NOT NULL DEFAULT '0' COMMENT '0-否：黑名单，1-是：白名单',
                        `create_time` datetime DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
                        `update_time` datetime DEFAULT CURRENT_TIMESTAMP COMMENT '更新时间',
                        `is_delete` tinyint(1) NOT NULL DEFAULT '0' COMMENT '软删除',
                        PRIMARY KEY (`id`) USING BTREE
